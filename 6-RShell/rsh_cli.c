...
{
	int rt; int recv_size;
	char* cmd_buff = malloc(sizeof(char) * RDSH_COMM_BUFF_SZ);
	char* resp_buff = malloc(sizeof(char) * RDSH_COMM_BUFF_SZ);
	...
	while (1) {
		...
		// Make sure the buffers are properly empty
		memset(cmd_buff, 0, RDSH_COMM_BUFF_SZ);
		memset(resp_buff, 0, RDSH_COMM_BUFF_SZ);

		// Print the prompt
		printf(SH_PROMPT);

		// Grab the input from standard in
		if (fgets(cmd_buff, RDSH_COMM_BUFF_SZ, stdin) == NULL) {
			return client_cleanup(sock, cmd_buff, resp_buff, OK);
		}
		...
		rt = send(sock, cmd_buff, strlen(cmd_buff), 0);
		if (rt < 0) {
			perror("Send\n");
			return client_cleanup(sock, cmd_buff, resp_buff, ERR_RDSH_COMMUNICATION);
		}

		// Wait for the response, print it out as it comes
		while ((recv_size = recv(sock, resp_buff, RDSH_COMM_BUFF_SZ, 0)) > 0) {
			// Some sort of error 
			if (recv_size < 0) {
				return client_cleanup(sock, cmd_buff, resp_buff, ERR_RDSH_COMMUNICATION);
			}
			...
			if (recv_size == 0) {
				//FIXME this might need to be changed to return
				break;
			}

			// Check to see if the data received has the EOF character, in which case we break the loop
			if (resp_buff[recv_size - 1] == RDSH_EOF_CHAR) {
				resp_buff[recv_size - 1] = '\0'; // Null terminate the buffer for easier handling
				printf("%.*s", recv_size, resp_buff);
				break;
			}

			// Otherwise we just print what was received
			else {
				printf("%.*s", recv_size, resp_buff);
			}
		}

		// If the client sent the exit command or stop-server command, stop the client
		if (strcmp(cmd_buff, EXIT_CMD) == 0 || strcmp(cmd_buff, STOP_SVR_CMD) == 0) {
			break;
		}
...
int start_client(char *server_ip, int port){
	...
	int connect_return = connect(client_socket, (const struct sockaddr *) &addr, sizeof(struct sockaddr_in));
